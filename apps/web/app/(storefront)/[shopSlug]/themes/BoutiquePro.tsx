...
export default function BoutiquePro({ shop, produits }: Props) {
  ...
  return (
    <div style={{ backgroundColor: t.bg, color: t.text, minHeight: '100vh' }}>
              ...
              <div>
                ...
                {shop.isVerified && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs font-medium" style={{ color: t.accent }}>Certifiée</span>
                  </div>
                )}
              </div>
            </Link>
      ...
      {/* ── HERO TEXTE (si pas d'image hero) ── */}
      {!shop.heroImage && (
      ...
      {/* ── SECTION STATS CONFIANCE ── */}
      <div style={{ borderBottom: `1px solid ${t.border}` }}>
        <div className="max-w-6xl mx-auto px-4 py-5">
          <div className="grid grid-cols-3 gap-3 text-center">
            {[
              { valeur: produits.length + '+', label: 'Produits disponibles' },
              { valeur: 'CI',                  label: 'Livraison partout'     },
              { valeur: '100%',                label: 'Paiement a livraison'  },
            ].map((s, i) => (
              <div key={i} className="space-y-0.5">
                <p className="text-lg font-extrabold" style={{ color: t.accent }}>{s.valeur}</p>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <Link href="/recherche"
          ...
          <Search size={18} style={{ color: t.muted }} />
          <span className="text-sm" style={{ color: t.muted }}>Rechercher un produit...</span>
        </Link>
      </div>
              ...
              return (
                <Link key={produit._id} href={`/produits/${produit._id}`}
                  ...
                  <div className="p-4 space-y-2 flex-1 flex flex-col justify-between">
                    ...
                    {/* CTA hover plein */}
                    <div
                      className="w-full py-2 rounded-xl text-xs font-bold text-center transition-all mt-1"
                      style={{ backgroundColor: `${t.accent}15`, color: t.accent }}
                      onMouseEnter={e => {
      ...
      <div style={{ borderTop: `1px solid ${t.border}` }}>
        <div className="max-w-6xl mx-auto px-4 py-14 space-y-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <ListeAvis
              shopSlug={shop.slug} type="boutique"
