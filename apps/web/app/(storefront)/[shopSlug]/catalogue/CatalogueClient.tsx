...
export default function CatalogueClient({ shop, categories, produits }: Props) {
  ...
  const produitsFiltres = useMemo(() => {
    ...
    if (categorie) {
      liste = liste.filter(p => p.categoryId === categorie);
    }

    liste = liste.filter(p =>
      p.price >= filtrePrix[0] && p.price <= filtrePrix[1]
    );
    ...
    <div style={{ backgroundColor: t.bg, color: t.text, minHeight: '100vh' }}>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-xl border text-sm
                         font-medium transition-colors lg:hidden"
              style={{
                backgroundColor: filtresOuverts ? t.accent : t.surface,
                borderColor:     filtresOuverts ? t.accent : t.border,
                color:           filtresOuverts ? '#fff'   : t.text,
              }}
            >
        ...
        <div className="flex gap-6">
          ...
          {/* ── FILTRES SIDEBAR ── */}
          <aside
            className={`flex-shrink-0 w-64 space-y-6
                        ${filtresOuverts ? 'block' : 'hidden'} lg:block`}
          >
            <div
              className="rounded-2xl border p-5 space-y-5 sticky top-20"
            ...
            >
                  ...
                  <div className="space-y-1">
                    ...
                    {categories.map(cat => {
                      ...
                      return (
                        <button
                          key={cat._id}
                          onClick={() => { setCategorie(cat._id); setPage(1); }}
                          className="w-full text-left px-3 py-2 rounded-lg text-sm
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  value={filtrePrix[1]}
                  onChange={e => {
                    setFiltrePrix([filtrePrix[0], Number(e.target.value)]);
                    setPage(1);
                  }}
              ...
              <div className="space-y-2">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" className="rounded" style={{ accentColor: t.accent }} />
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((n, i) => n === '...'
                    ...
                    : (
                      <button
                        key={n}
                        onClick={() => changerPage(n as number)}
