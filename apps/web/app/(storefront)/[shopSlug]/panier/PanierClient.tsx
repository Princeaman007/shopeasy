...
export default function PanierClient({ shop }: Props) {
  ...
  const modifierQuantite = (cle: string, delta: number) => {
    const liste = articles.map(a => {
      ...
      return { ...a, quantite: Math.max(1, a.quantite + delta) };
    });
    sauvegarder(liste);
  };
  ...
  const commanderWhatsApp = () => {
    ...
    const lignes = articles.map(a => {
      const variantesTexte = Object.entries(a.variantes)
        .map(([k, v]) => `${k}: ${v}`)
        .join(', ');
    ...
    const message = encodeURIComponent(
      ...
      `${lignes}\n\n` +
      (promoApplique ? `🏷️ Code promo : ${promoApplique.code} (-${formatFcfa(reduction)})\n` : '') +
          ...
          <>
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => modifierQuantite(article.cle, -1)}
            ...
            {shop.planType === 'premium' && (
              ...
              >
                ...
                {promoApplique ? (
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-transparent border rounded-xl px-4 py-2.5
                                 text-sm outline-none font-mono tracking-widest uppercase"
                ...
                {promoErreur && (
                  <p className="text-xs text-red-400">{promoErreur}</p>
                )}
              </div>
            )}
            ...
            >
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span style={{ color: t.text }}>{formatFcfa(sousTotal)}</span>
                </div>
                ...
                {reduction > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-green-400">-{formatFcfa(reduction)}</span>
                  </div>
                )}
              ...
              <p className="text-xs text-center" style={{ color: t.muted }}>
                Tu seras redirigé vers WhatsApp pour finaliser ta commande
              </p>
            </div>
          </>
        )}
